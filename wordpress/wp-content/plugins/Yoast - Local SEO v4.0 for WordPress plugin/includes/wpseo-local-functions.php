...
function wpseo_local_show_address( $atts ) {
	...
	*/
	...
	if ( true === has_post_thumbnail( $atts['id'] ) ) {
		$output .= '<meta itemprop="image" content="' . esc_url( get_the_post_thumbnail_url( $atts['id'] ) ) . '">';
	}
	...
	// Remove first comma from oneline addresses when business name is hidden or the company logo is shown.
	if ( ! empty( $address_format_output ) && true === $atts['oneline'] && ( true === $atts['hide_name'] || ( false === $atts['hide_name'] && true == $atts['show_logo'] && ! empty( $business_location_logo ) ) ) ) {
...
function wpseo_local_show_map( $atts ) {
	...
	// Check to not show draft locations to non-logged in users.
	if ( wpseo_has_multiple_locations() && ( 'publish' != get_post_status( $atts['id'] ) && 'all' != $atts['id'] && ! current_user_can( 'edit_posts' ) ) ) {
		return '';
	}
	...
	if ( $location_array_str != '' ) {
		...
		$wpseo_map .= '<script type="text/javascript">
			...
			function wpseo_map_init' . ( ( $map_counter != 0 ) ? '_' . $map_counter : '' ) . '() {
				...
				map_' . $map_counter . ' = wpseo_show_map( location_data, ' . $map_counter . ', ' . $center_lat . ', ' . $center_long . ', ' . $atts['zoom'] . ', "' . $atts['map_style'] . '", "' . $atts['scrollable'] . '", "' . $atts['draggable'] . '", "' . $atts['default_show_infowindow'] . '", "' . is_admin() . '" );
...
function wpseo_enqueue_geocoder() {
	...
	if ( $wpseo_enqueue_geocoder ) {
		...
		$script_name = 'wp-seo-local-frontend-400.min.js';
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
			$script_name = 'wp-seo-local-frontend-400.js';
		}
