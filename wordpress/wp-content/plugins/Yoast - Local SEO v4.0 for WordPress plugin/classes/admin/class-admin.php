	...
	class WPSEO_Local_Admin {
		...
		function config_page_styles() {
			...
			if ( ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] == 'wpseo_local' ) || ( in_array( $pagenow, array(
				...
				wp_enqueue_style( 'wpseo-local-admin-css', plugins_url( '../styles/admin-360' . $css_ext, dirname( __FILE__ ) ), WPSEO_LOCAL_VERSION );
			}
			else {
		...
		function config_page_scripts() {
			...
			wp_enqueue_script( 'wpseo-local-global-script', plugins_url( '../js/wp-seo-local-global-390' . $js_ext, dirname( __FILE__ ) ), array( 'jquery' ), WPSEO_LOCAL_VERSION, true );
