							...
							if ( ! is_wp_error( $geo_data ) && ! empty( $geo_data->results[0] ) ) {
								...
								if ( $geo_data->get_error_code() == 'wpseo-query-limit' ) {
									...
									if ( ! empty( $last_imported ) ) {
										$this->messages[] = array(
											'type'    => 'warning',
											/* translators: First %s extends to the eidt link for the last imported location; Second %s extends to that location's title; Third %s closes the link tag opened in the first %s */
											'content' => sprintf( __( 'The last successfully imported location is <a href="%s" title="%s">%s</a>', 'yoast-local-seo' ), get_edit_post_link( $last_imported ), get_the_title( $last_imported ), get_the_title( $last_imported ) ),
										);
									}
									break;
								}
